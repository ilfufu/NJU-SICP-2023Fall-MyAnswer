...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        myoperator=self.operator.eval(env)
        myoperands=[i.eval(env) for i in self.operands]
        return myoperator.apply(myoperands)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        p_copy.update(zip(self.parameters,arguments))
