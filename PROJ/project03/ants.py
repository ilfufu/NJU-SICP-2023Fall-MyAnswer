...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place=self.place
        distance=0
        while (not place.is_hive) and nearest_bee is None:
...
class BodyguardAnt(ContainerAnt):
    ...
    implemented = True   # Change to True to view in the GUI
    def __init__(self,*args, **kwargs):
        ContainerAnt.__init__(self,*args, **kwargs,health=2)
    # END PROBLEM 8
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        cplace=self.place.exit
        while cplace is not None:
            if cplace.ant is not None:
             if (cplace.ant.is_container and cplace.ant.ant_contained is not None and cplace.ant.ant_contained not in self.doubled):
                cplace.ant.ant_contained.buff()
                self.doubled.append(cplace.ant.ant_contained)
             if cplace.ant not in self.doubled:
                cplace.ant.buff()
             self.doubled.append(cplace.ant)
            cplace=cplace.exit
