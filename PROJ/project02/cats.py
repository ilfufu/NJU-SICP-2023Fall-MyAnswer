...
def choose(paragraphs, select, k):
    ...
    for i in range(len(paragraphs)):
        if count != 0 and select(paragraphs[i]):
            count-=1
        if count == 0:
            return paragraphs[i]
    return ''
...
def accuracy(typed, source):
    ...
    min_len=min(tlen,slen)
    count=0
    
    if tlen == 0 and slen != 0:
        return 0.0
    elif tlen != 0 and slen ==0:
        return 0.0
    elif tlen ==0 and slen ==0:
        return 100.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    lis_copy=[]
    for i in range(len(word_list)):
        lis_copy.append(diff_function(typed_word,word_list[i],limit))
    ...
    temp_values=lis_copy[0]
    len1=len(lis_copy)
    for i in range(len1):
        temp = lis_copy[i]
        if temp<temp_values:
            temp_values=lis_copy[i]
    for i in range(len1):
        if lis_copy[i]==temp_values:
            num=i
            break
...
def minimum_mewtations(start, goal, limit):
    ...
    if start==goal:
        ...
        addition=minimum_mewtations(start,goal[1:],limit-1)+1
        remove=minimum_mewtations(start[1:],goal[1:],limit-1)+1
        substitute=minimum_mewtations(start[1:],goal,limit-1)+1
        return min(addition,min(remove,substitute))
...
def report_progress(typed, prompt, user_id, upload):
    ...
    diction={'id':user_id,'progress':rate}
    upload(diction)
    return rate
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    copy=[[0]*(len(words)) for _ in range(len(times_per_player))]
    for i in range(len(times_per_player)):
        for j in range(len(words)):
            copy[i][j]=times_per_player[i][j+1]-times_per_player[i][j]
    return {'words':words,'times':copy}
...
def fastest_words(game):
    ...
    for i in word_indices:
        player_temp=0
        min_temp=time(game,player_temp,i)
        for j in player_indices:
            if time(game,j,i)<min_temp:
                min_temp=time(game,j,i)
                player_temp=j
        return_list[player_temp].append(get_word(game,i))
