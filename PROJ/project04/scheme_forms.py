...
def do_if_form(expressions, env):
    ...
    if is_scheme_true(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, True) #here add True
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, True) #here add True
...
def do_and_form(expressions, env):
    ...
    while expressions.rest != nil:
        ...
        if not is_scheme_true(v):
            return v
        else:
            expressions = expressions.rest
    return scheme_eval(expressions.first, env, True) #here add True
    #  END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    values = bindings.map(lambda item: scheme_eval(item.rest.first, env)) #I change here
