...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def problem(i, sum, point):
        if i == n:
          return sum
        elif i % 6 == 0 or '6' in str(i):
          return problem(i+1, sum-point, -point)
        else:
          return problem(i+1, sum+point, point)
    return problem(1, 1, 1)
...
def missing_digits(n):
    ...
    num2 = n%10
    num1 = (n//10)%10
    if num2-num1==0 or num2-num1==1:
       return 0+missing_digits(n//10)
    else:
       return (num2-num1-1)+missing_digits(n//10)
...
def count_change(total, next_money):
    ...
    """
    "*** YOUR CODE HERE ***"
    #conclusion：if：特殊情况 else：递推公式
    
    money_list = [0,1]
    def f(total,x):
        if x<total:
            if next_money(x) is not None and next_money(x)>total:
                return 0 
            else:
                x=next_money(x)
                if x is None:
    ...
    def count(total,num):
        ...
        return count(total-money_list[num],num)+count(total,num-1)
...
def move_stack(n, start, end):
    ...
    pole is either empty or has a top d isk larger than the top n start disks.
    ...
    if n == 1:
        ...
        othern = (6 - start - end)
        move_stack(n - 1, start, othern)
        print('Move the top disk from rod',start,'to rod',end)
        move_stack(n - 1, othern, end)
